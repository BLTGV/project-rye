...
'Advisory identity lookup. Read-only: returns a verdict and candidates, writes nothing, blocks nothing. Fuzzy label matches never yield match. SECURITY INVOKER, so a node hidden from the caller reports new (rls-visibility-contract D3 is not implemented).';
...
'Follows node_merges transitively to the surviving node. Returns the input when it was never merged. Raises on a merge cycle. SECURITY INVOKER: reads node_merges under the caller''s RLS, so a chain through a node the caller cannot see stops at the last visible link.';
