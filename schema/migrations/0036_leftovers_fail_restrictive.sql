...
DECLARE
    ...
    v_roles     text[];
BEGIN
    v_roles := assertion_settle_roles(p_assertion_type);

    IF v_roles IS NULL THEN
        v_canonical := canonical_type('assertion_type', p_assertion_type);
        IF v_canonical IS DISTINCT FROM p_assertion_type THEN
...
COMMENT ON FUNCTION settle_gate(text) IS
    'Answer {assertion_type, gated, gated_as, allowed_roles, current_role, may_settle} for an assertion type. Gated when the spelling given has a settle row or its canonical spelling does, which is the rule record_assertion() applies; gated_as names the other spelling and is null when the spelling given is itself the gated one. STABLE, SECURITY INVOKER, writes nothing. Call it before offering to record configuration.';
