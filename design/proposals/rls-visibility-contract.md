...
work. The default-safe half is implemented alongside it: D1 in
`schema/migrations/0032_graph_traversal.sql` (work/013), D4 with the identity
resolution work (work/014). D2 is a follow-up. D3 is blocked on a finding
recorded below: the definer probe it assumed does not bypass RLS on every
deployment.
...
`match | ambiguous | new` only, and `tests/security/04_identity_visibility.sql`
pins the current behavior so that adopting D3 changes the test deliberately.
(`tests/security/03_...` is the traversal suite; the identity suite took the
next free number.)
