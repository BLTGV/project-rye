...
Codes are generated by `generate_crm_code()` (see [Functions](/docs/model/functions/)) and stored in both `properties->>'code'` and `external_id` with `external_source = 'internal'`.
...
CRM access builds on the core RLS infrastructure (see [Security](/docs/model/security/)).
