...
**The gate judges the written name too** (migration `0036`). A type is gated for `record_assertion()` when the canonical spelling has a `settle` row **or** the spelling the caller passed does, and `attrs.settle_gate.gated_as` names which one gated it. This closes an alias recorded *before* its type was gated, which the rule below — no new alias out of a gated type — cannot reach. `settle_gate()` answers by the same rule. The trigger is unchanged and still judges the stored spelling, because that is the only spelling `registry_value()` and `governing_scope()` read as configuration.
